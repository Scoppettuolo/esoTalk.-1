/**
 * This file is part of the esoTalk project, a derivative of esoTalk.
 * It has been modified by several contributors.  (contact@geteso.org)
 * Copyright (C) 2023 esoTalk, esoTalk.  <https://github.com/Scoppettuolo>
 * 
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * (at your option) any later version of the GNU Affero General Public License as published by the Free Software Foundation, either version 3 of the License, or
...
// Get elements by class name (native API with fallback).
function getElementsByClassName(parent, className) {
	parent = parent || document;
	if (parent.getElementsByClassName) {
		return Array.prototype.slice.call(parent.getElementsByClassName(className));
	}
	var found = [];
...
function getScrollDimensions() {
	...
	return [
		...
		Math.max(de.scrollHeight || 0, b.scrollHeight || 0)
	];
}
...
init: function() {
	...
	// Grow any auto-expand textarea (reply + inline post edit)
	if (document.addEventListener) {
		document.addEventListener("focusin", function(e) {
			var t = e.target;
			if (t && t.tagName === "TEXTAREA" && (t.className || "").indexOf("auto-expand") >= 0)
				Conversation.autoExpandTextarea(t);
		});
	}
...
autoExpandTextarea: function(el) {
	if (!el) return;
	var fit = function() {
		el.style.height = "auto";
		var h = Math.min(Math.max(el.scrollHeight, 72), 420);
		el.style.height = h + "px";
	};
	if (el.addEventListener) {
		el.addEventListener("input", fit, false);
		el.addEventListener("keyup", fit, false);
	} else {
		el.onkeyup = fit;
	}
	if (el.value && el.value.length) fit();
},

// Expand the reply area exactly once and focus its textarea.
showReply: function() {
	var reply = getById("reply");
	var textarea = getById("reply-textarea");
	if (!reply) return;
	if (!Conversation.replyShowing) {
		Conversation.replyShowing = true;
		reply.className = reply.className.replace(/\s*(replyPlaceholder|reply-expanded)\b/g, "") + " reply-expanded";
	}
	if (textarea && textarea.focus && document.activeElement !== textarea) textarea.focus();
},

// Collapse an empty reply area, allowing the CSS transition to animate the change.
hideReply: function() {
	var reply = getById("reply");
	var textarea = getById("reply-textarea");
	var value = textarea ? textarea.value.replace(/^\s+|\s+$/g, "") : "";
	if (!reply || !Conversation.replyShowing || value) return;
	Conversation.replyShowing = false;
	reply.className = reply.className.replace(/\s*(replyPlaceholder|reply-expanded)\b/g, "") + " replyPlaceholder";
...
initReply: function() {
		...
		var reply = getById("reply");
		var textarea = getById("reply-textarea");
		var hasContent = !!(textarea && textarea.value && textarea.value.replace(/^\s+|\s+$/g, ""));
		Conversation.replyShowing = hasContent;
		
		// Compact box that expands on click/focus; drafts remain expanded so their content is never hidden.
		Conversation.autoExpandTextarea(textarea);
		if (reply) {
			reply.className = reply.className.replace(/\s*(replyPlaceholder|reply-expanded)\b/g, "") + (hasContent ? " reply-expanded" : " replyPlaceholder");
			reply.onclick = function(e) {
				e = e || window.event;
				Conversation.showReply();
				if (e && e.stopPropagation) e.stopPropagation();
			};
		}
		if (textarea) textarea.onfocus = function() { Conversation.showReply(); };
		if (!Conversation.replyDocumentClickBound) {
			var collapseReply = function(e) {
				e = e || window.event;
				var target = e.target || e.srcElement;
				while (target) {
					if (target === getById("reply")) return;
					target = target.parentNode;
				}
				Conversation.hideReply();
			};
			if (document.addEventListener) document.addEventListener("click", collapseReply, false);
			else document.attachEvent("onclick", collapseReply);
			Conversation.replyDocumentClickBound = true;
		}

		// Disable the "post reply" button if there's not a draft. Disable the save draft button regardless.
		if (!textarea.value) {disable(getById("postReply")); disable(getById("discardDraft"));}
		disable(getById("saveDraft"));
		
		// Add event handlers on the textarea to enable/disable buttons.
		var updateReplyState = function() {
			var value = textarea.value.replace(/^\s+|\s+$/g, "");
			if (value) {
				enable(getById("saveDraft")); enable(getById("postReply")); Conversation.editingReply = true;
				Conversation.showReply();
			}
			else {disable(getById("saveDraft")); disable(getById("postReply")); Conversation.editingReply = false;}
		};
		textarea.onkeyup = updateReplyState;
		textarea.oninput = updateReplyState;
