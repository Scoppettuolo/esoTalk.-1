...
$versions = array("eso" => "0");
if (file_exists(PATH_CONFIG."/versions.php")) {
	include PATH_CONFIG."/versions.php";
}
if (!isset($versions["eso"]) || $versions["eso"] != ESO_VERSION) {
...
if (isset($_SERVER["HTTP_HOST"]) && !empty($config["baseURL"])) {
	...
	if (is_array($urlParts) && isset($urlParts["host"])) {
		...
		if ($urlParts["host"] != $_SERVER["HTTP_HOST"]) {
			header("Location: " . $config["baseURL"] . substr($_SERVER["REQUEST_URI"] ?? "", strlen($path)));
			exit;
		}
	}
}
...
if (session_id()) {
	...
	if (isset($_SESSION["ip"]) && isset($_SESSION["time"]) && isset($_SESSION["userAgent"])) {
		...
		// Accept legacy md5 fingerprints so existing sessions keep working until they expire.
		$storedUa = $_SESSION["userAgent"] ?? "";
		$userAgentMismatch = ($storedUa !== $uaHash && $storedUa !== md5($_SERVER["HTTP_USER_AGENT"] ?? ""));
...
// Migrate legacy non-ASCII language pack names to ASCII filenames.
$langAliases = array(
...
$preferredLang = !empty($_SESSION["user"]["language"]) ? $_SESSION["user"]["language"] : ($config["language"] ?? "English (casual)");
if (isset($langAliases[$preferredLang])) {
	$preferredLang = $langAliases[$preferredLang];
}
$eso->language = sanitizeFileName((!empty($preferredLang) and file_exists(PATH_LANGUAGES . "/{$preferredLang}.php")) ? $preferredLang : ($config["language"] ?? "English (casual)"));
if (file_exists(PATH_LANGUAGES . "/$eso->language.php")) include PATH_LANGUAGES . "/$eso->language.php";
...
require PATH_CONFIG . "/skin.php";
if (file_exists(PATH_SKINS . "/{$config["skin"]}/skin.php")) include_once PATH_SKINS . "/{$config["skin"]}/skin.php";
...
require PATH_CONFIG . "/plugins.php";
foreach ($config["loadedPlugins"] as $v) {
	$v = sanitizeFileName($v);
	if (file_exists(PATH_PLUGINS . "/$v/plugin.php")) include_once PATH_PLUGINS . "/$v/plugin.php";
	if (class_exists($v)) {
		$eso->plugins[$v] = new $v;
