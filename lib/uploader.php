...
public $lastFileInfo = false;

public function iniToBytes($value)
{
	$l = substr($value, -1);
...
public function downloadFromUrl($url, $allowedTypes = array())
{
	$this->lastError = false;
	$this->lastFileInfo = false;
...
public function validateImageMagicBytes($filePath, $expectedMimeType)
{
...
public function validateImage($filePath, $allowedTypes = array())
{
	if (!file_exists($filePath) || !is_readable($filePath)) return false;
...
public function createImageResource($filePath, $mimeType)
{
	switch ($mimeType) {
		case "image/jpeg":
		case "image/pjpeg":
			if (!self::gdAvailable()) return false;
...
public function resizeImage($curWidth, $curHeight, $maxWidth = null, $maxHeight = null, $exactWidth = null, $exactHeight = null)
{
...
public function saveImageResource($image, $destination, $format, $quality = 85)
{
	switch ($format) {
		case "jpg":
		case "jpeg":
...
public function saveAsImage($source, $destination, $options = array())
{
	$this->lastError = false;
...
public function createThumbnail($source, $destination, $thumbnailOptions, $mimeType, $curWidth, $curHeight)
{
