...
public function revert($string, $formatters = false)
{
	// Work out which formatters are going to be used.
	if (is_array($formatters)) $formatters = array_intersect(array_keys($this->modes), $formatters);
	else $formatters = array_keys($this->modes);
...
public function text($match, $state) {
 	$this->output .= $match;
 	return true;
}
...
public function getModes($modes, $exclude = false)
{
	$newModes = array();
...
public function paragraph($match, $state)
{
...
// Insert a linebreak.
public function linebreak($match, $state)
{
...
public $modes = array("bold_tag_b", "bold_tag_strong", "bold_bbcode", "bold_wiki");
public $revert = array("<b>" => "&lt;b&gt;", "</b>" => "&lt;/b&gt;");
...
public function bold($match, $state)
{
	switch ($state) {
...
public $formatter;
public $modes = array("italic_tag_i", "italic_tag_em", "italic_bbcode", "italic_wiki");
public $revert = array("<i>" => "&lt;i&gt;", "</i>" => "&lt;/i&gt;");
...
public function italic($match, $state)
{
	switch ($state) {
...
public $formatter;
public $modes = array("strike_html", "strike_bbcode", "strike_wiki");
public $revert = array("<del>" => "&lt;s&gt;", "</del>" => "&lt;/s&gt;");
...
public function strike($match, $state)
{
	switch ($state) {
...
public $formatter;
public $modes = array("superscript", "subscript");
public $revert = array("<sup>" => "&lt;sup&gt;", "</sup>" => "&lt;/sup&gt;", "<sub>" => "&lt;sub&gt;", "</sub>" => "&lt;/sub&gt;");
...
public function superscript($match, $state)
{
	switch ($state) {
...
public function subscript($match, $state)
{
	switch ($state) {
...
public $formatter;
public $modes = array("heading_html", "heading_bbcode", "heading_wiki");
public $revert = array("<h3>" => "&lt;h1&gt;", "</h3>" => "&lt;/h1&gt;\n\n");
...
public function heading($match, $state)
{
	switch ($state) {
...
public $formatter;
public $modes = array("quote_html", "quote_bbcode");
public $revert = array(
	"<blockquote>" => "\n&lt;blockquote&gt;",
	"<blockquote><p>" => "\n&lt;blockquote&gt;",
	"</blockquote>" => "\n&lt;/blockquote&gt;\n\n",
...
public function cite($match, $state)
{
	switch ($state) {
...
public function quote($match, $state)
{
	switch ($state) {
...
public $formatter;
public $modes = array("pre_html_block", "code_html_block", "code_bbcode_block");
public $revert = array("<pre>" => "&lt;pre&gt;", "</pre>" => "&lt;/pre&gt;\n\n");
...
public function fixedBlock($match, $state)
{
	switch ($state) {
...
public $formatter;
public $modes = array("pre_html_inline", "code_html_inline", "code_bbcode_inline");
public $revert = array("<code>" => "&lt;pre&gt;", "</code>" => "&lt;/pre&gt;");
...
public function fixedInline($match, $state)
{
	switch ($state) {
...
public $formatter;
public $modes = array("link_html", "link_bbcode", "link_wiki", "postLink", "conversationLink");
...
public function email($match, $state)
{
...
public function postLink($match, $state)
{
	switch ($state) {
		case LEXER_ENTER:
...
public function conversationLink($match, $state)
{
	switch ($state) {
		case LEXER_ENTER:
...
public function url($match, $state)
{
	global $config;
	$protocol = "";
...
public function link($match, $state)
{
	global $config;
	switch ($state) {
		case LEXER_ENTER:
...
public $formatter;
public $modes = array("image_html", "image_bbcode1", "image_bbcode2");
...
public function image_html($match, $state)
{
...
public function image_bbcode1($match, $state)
{
...
// Add an image tag to the output by interpreting a BBCode image tag.
public function image_bbcode2($match, $state)
{
...
public function image($src, $alt = "", $title = "")
{
...
public function video_html($match, $state)
{
	if (preg_match("`src=(&#39;|&quot;)(https?:\/\/[^\s]+?)\\1`", $match, $src)) $src = $src[2];
	else {
		$this->formatter->output .= $match;
...
public function video($src, $title = "")
{
	if (!empty($title)) $titleQuote = strpos($title, "&#39;") === false ? "'" : '"';
...
public function audio_html($match, $state)
{
	if (preg_match("`src=(&#39;|&quot;)(https?:\/\/[^\s]+?)\\1`", $match, $src)) $src = $src[2];
	else {
		$this->formatter->output .= $match;
...
public function audio($src, $title = "")
{
	if (!empty($title)) $titleQuote = strpos($title, "&#39;") === false ? "'" : '"';
...
public $formatter;
public $modes = array("blockList");

public $listStack = array();
public $initialDepth = 0;
...
public function blockList($match, $state) {
	switch ($state) {
...
public function uli($match, $state)
{
...
public function oli($match, $state)
{
...
public function unorderedList($match, $state)
{
	switch ($state) {
...
public function orderedList($match, $state)
{
	switch ($state) {
...
public function horizontalRule($match, $state)
{
...
public function entity($match, $state)
{
