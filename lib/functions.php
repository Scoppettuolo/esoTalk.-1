...
// Escape values that will be written inside a double-quoted PHP or JavaScript string.
// Backslashes must be escaped too: Windows paths such as C:\\xampp\\... otherwise
// turn sequences like \\t or \\n into control characters when config.php is included again.
...
function cookieIp()
{
	// Always return a non-negative integer suitable for SQL (ip column is int unsigned).
	$raw = $_SERVER["REMOTE_ADDR"] ?? "127.0.0.1";
	$raw = trim((string)$raw);
	...
	if (filter_var($raw, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
		$long = ip2long($raw);
		if ($long !== false) {
			// ip2long can return negative on 32-bit; normalize to unsigned
			if ($long < 0) $long = sprintf("%u", $long);
			return (int)$long;
		}
	}

	// IPv6 (e.g. ::1 on XAMPP/Windows): map to a stable 32-bit value
	if (filter_var($raw, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)) {
		if ($raw === "::1") return 2130706433; // 127.0.0.1
		$bin = @inet_pton($raw);
		if ($bin !== false && strlen($bin) === 16) {
			// Use last 4 bytes as unsigned int
			$parts = unpack("N", substr($bin, 12));
			return (int)($parts[1] ?? 0);
		}
	}

	// Last resort: hash to unsigned 32-bit
...
function curLink()
{
	...
	$curLink = preg_replace("|^" . preg_quote($path, "|") . "|", "", $_SERVER["REQUEST_URI"] ?? "");
	return $curLink;
}
...
function validateEmail(&$email)
{
	...
	elseif ($eso->db->fetchOne("SELECT 1 FROM {$config["tablePrefix"]}members WHERE email=? AND account!='Unvalidated'", "s", $email)) return "emailTaken";
}
...
function validateName(&$name)
{
	...
	if ($eso->db->fetchOne("SELECT 1 FROM {$config["tablePrefix"]}members WHERE name=? AND account!='Unvalidated'", "s", $name))
		return "nameTaken";
}
...
function relativeTime($then)
{
	...
	if (empty($then)) return $language["Never"] ?? "Never";
...
function checkFloodControl($action, $rateLimit, $sessionKey, $errorMessage, $memberId = null)
{
	...
	// However, if we don't have a record in the session, use the MySQL actions table.
		...
		$ip = (int)$ip;
		$cnt = (int)$eso->db->fetchOne("SELECT COUNT(*) FROM {$config["tablePrefix"]}actions WHERE ip=? AND action=? AND time>UNIX_TIMESTAMP()-60", "is", $ip, $action);
		if ($cnt >= $rateLimit) {
			$eso->message($errorMessage, true, 60);
			return false;
		}
		// Log this attempt in the actions table.
		$memberIdValue = $memberId !== null ? (int)$memberId : "NULL";
		if ($memberId !== null) {
			$eso->db->queryPrepared("INSERT INTO {$config["tablePrefix"]}actions (ip, memberId, action, time) VALUES (?, ?, ?, UNIX_TIMESTAMP())", "iis", $ip, (int)$memberId, $action);
		} else {
			$eso->db->queryPrepared("INSERT INTO {$config["tablePrefix"]}actions (ip, memberId, action, time) VALUES (?, NULL, ?, UNIX_TIMESTAMP())", "is", $ip, $action);
		}
		// Proactively clean the actions table of attempts older than 60 seconds.
		$eso->db->queryPrepared("DELETE FROM {$config["tablePrefix"]}actions WHERE action=? AND time<UNIX_TIMESTAMP()-60", "s", $action);
	}
...
function hashPassword($password, $salt = null, $config = null)
{
	if ($config === null) {
		global $config;
	}
	$method = is_array($config) ? ($config["hashingMethod"] ?? "bcrypt") : "bcrypt";
	if ($method === "bcrypt") {
...
// Auto-detects bcrypt hashes ($2y$/$2a$/$2b$) regardless of config for upgrade safety.
function verifyPassword($password, $hash, $salt = "", $config = null)
{
	if ($config === null) {
		global $config;
	}
	...
	// Modern bcrypt / argon-style hashes
	if (strncmp($hash, '$2y$', 4) === 0 || strncmp($hash, '$2a$', 4) === 0 || strncmp($hash, '$2b$', 4) === 0) {
		return password_verify($password, $hash);
	}
	if (strncmp($hash, '$argon', 6) === 0) {
		return password_verify($password, $hash);
	}
	// Legacy esoTalk md5(salt + password) — still accepted so old accounts can log in and upgrade
	$legacy = md5((string)$salt . $password);
...
/**
 * Build a SQL fragment that limits conversations the current user may see.
 * Member IDs are cast to int; account group names are escaped.
 * Safe for embedding in larger dynamic SELECT builders.
 */
function conversationAccessPredicate($eso, $config, $conversationAlias = "c", $statusAlias = "s")
{
	$c = $conversationAlias;
	$s = $statusAlias;
	$prefix = $config["tablePrefix"] ?? "";
	if (empty($eso->user)) {
		return "{$c}.posts>0 AND {$c}.private=0";
	}
	$memberId = (int)$eso->user["memberId"];
	$account = $eso->db->escape((string)$eso->user["account"]);
	return "{$c}.startMember={$memberId} OR ({$c}.posts>0 AND ({$c}.private=0 OR {$s}.allowed OR (SELECT allowed FROM {$prefix}status WHERE conversationId={$c}.conversationId AND memberId='{$account}')))";
}

/**
 * Sanitize a list of IDs from user input into a comma-separated list of ints.
 * Returns empty string if none valid.
 */
...
/**
 * Profile color hex map (classic esoTalk pastel palette).
 */
function profileColorHex($colorId)
{
	static $map = array(
		1 => "dfdfdf", 2 => "d2d2d2", 3 => "cf9eff", 4 => "b29cff",
		5 => "a5b1ff", 6 => "adccff", 7 => "b4e7ff", 8 => "93fff1",
		9 => "a4ffd4", 10 => "93ffae", 11 => "b5ff97", 12 => "d4ff95",
		13 => "ebff99", 14 => "fff391", 15 => "ffdb75", 16 => "ffcd92",
		17 => "ffc19b", 18 => "ffa3af", 19 => "ffafdc", 20 => "eba5ff",
		21 => "c0c0db", 22 => "abcbcd", 23 => "b8d9c2", 24 => "c8d9b8",
		25 => "e5e2ba", 26 => "e7d0bb", 27 => "cdc5bd",
	);
	$id = max(1, min(27, (int)$colorId));
	return $map[$id] ?? "dfdfdf";
}

/**
 * Relative luminance 0–1 for a 6-char hex color (no #).
 */
...
/**
 * CSS class for post header text contrast against profile color.
 * hdr-light-text = white text (dark backgrounds)
 * hdr-dark-text  = dark text (light backgrounds)
 */
function postColorContrastClass($colorId)
{
	$L = colorLuminance(profileColorHex($colorId));
	return $L < 0.35 ? "hdr-light-text" : "hdr-dark-text";
}
