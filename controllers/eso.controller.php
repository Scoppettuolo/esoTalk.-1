...
public $user;
public $action;
public $allowedActions = array("admin", "conversation", "feed", "forgot-password", "join", "online", "post", "profile", "search", "settings");
public $controller;
public $view = "wrapper.php";
public $language;
public $skin;
public $head = "";
public $scripts = array();
public $jsLanguage = array();
public $jsVars = array();
...
public $memberGroups = array("Administrator", "Moderator", "Member", "Suspended");
public $bar = array("left" => array(), "right" => array());
...
{	
	...
	// Support both legacy keys (mysqlHost/...) and short keys (dbHost/...).
		$host = $config["mysqlHost"] ?? $config["dbHost"] ?? "localhost";
		$user = $config["mysqlUser"] ?? $config["dbUser"] ?? "";
		$pass = $config["mysqlPass"] ?? $config["dbPass"] ?? "";
		$name = $config["mysqlDB"] ?? $config["dbName"] ?? "";
		$driver = $config["databaseDriver"] ?? "mysqli";
		$sqlitePath = $config["sqlitePath"] ?? (PATH_CONFIG . "/esotalk.sqlite");
		$this->db = new Database($host, $user, $pass, $name, $config["characterEncoding"] ?? "utf8mb4", $driver, $sqlitePath);
		$this->db->eso =& $this;
		if (method_exists($this->db, "connected") && !$this->db->connected()) {
			$err = method_exists($this->db, "connectError") ? $this->db->connectError() : "Could not connect to the database.";
			$this->fatalError(!empty($config["verboseFatalErrors"]) ? $err : "Could not connect to the database. Check config/config.php.", "database");
		} elseif (method_exists($this->db, "connectError") && $this->db->connectError()) {
			$this->fatalError(!empty($config["verboseFatalErrors"]) ? $this->db->connectError() : "", "database");
		}
...
public function init()
{
	...
	if (is_file(PATH_CONFIG . "/custom.css") && filesize(PATH_CONFIG . "/custom.css") > 0) $this->addCSS("config/custom.css");
...
public function login($name = false, $password = false, $hash = false)
{
	global $config;
	...
	if ($name and $password) {
		...
		if ($row) {
			...
			if (verifyPassword($password, $dbPassword, $salt, $config)) {
				...
				// Wrong password — keep $hash false so login fails cleanly (do NOT generate a new hash).
				$hash = false;
			}
		} else {
			$hash = false;
	...
	if (($name or ($memberId = (int)$memberId)) and $hash !== false) {
		...
		if ($data) {
			...
			// Upgrade legacy md5 hashes to password_hash() on successful login.
			$stored = (string)$data["password"];
			$isModern = (strncmp($stored, '$2y$', 4) === 0 || strncmp($stored, '$2a$', 4) === 0
				|| strncmp($stored, '$2b$', 4) === 0 || strncmp($stored, '$argon', 6) === 0);
			if ($password && !$isModern) {
		...
		if (!isset($cookie)) $this->message("incorrectLogin", true, makeLink("forgot-password"));

	}
	
	// Didn't completely fill out the login form? Return an error.
	elseif ($name or $password)	$this->message("incorrectLogin", true, makeLink("forgot-password"));
...
public function getStatistics()
{
	...
	$result = $this->db->query("SELECT (SELECT COUNT(*) FROM {$config["tablePrefix"]}posts),
		...
		(SELECT COUNT(*) FROM {$config["tablePrefix"]}members WHERE UNIX_TIMESTAMP()-{$onlineExpire}<lastSeen)");
...
public function checkForUpdates()
{
	...
	writeConfigFile(PATH_CONFIG . "/lastUpdateCheck.php", '$lastUpdateCheck', time());
	
	// Get the latest version from geteso.org.
	if (($handle = @fopen("https://github.com/Scoppettuolo/latestVersion.txt", "r")) === false) return;
...
public function registerController($name, $file)
{
	if (@$_GET["q1"] == $name) {
		require_once $file;
		$this->action = $name;
...
public function fatalError($message, $esoSearch = "")
{
	global $language, $config;
...
public function addMessage($key, $class, $message)
{
	global $messages;
...
public function message($key, $disappear = true, $arguments = false)
{
...
public function htmlMessage($key, $arguments = false)
{
	global $messages;
	if (!isset($messages[$key]) || !is_array($messages[$key])) {
		return "<div class='msg warning'>Unknown message: " . htmlspecialchars((string)$key) . "</div>";
	}
	$m = $messages[$key];
	if (!empty($arguments)) {
		$fmt = is_array($arguments) ? @vsprintf($m["message"], $arguments) : @sprintf($m["message"], $arguments);
		if ($fmt !== false) $m["message"] = $fmt;
	}
	$class = isset($m["class"]) ? $m["class"] : "info";
...
public function addLanguageToJS()
{
	global $language;
	$args = func_get_args();
	foreach ($args as $key) {
...
public function addVarToJS($key, $val)
{
	$this->jsVars[$key] = $val;
}

// Add a JavaScript file to be included in the page.
public function addScript($script, $position = false)
{
...
public function head()
{
	...
	$head = "<!-- This page was generated by esoTalk (https://github.com/Scoppettuolo) -->\n";
...
public function addToBar($side, $html, $position = false)
{
	addToArray($this->bar[$side], $html, $position);
}

// Add a string of HTML to the page footer.
public function addToFooter($html, $position = false)
{
	addToArray($this->footer, $html, $position);
}

// Add a CSS file to be included on the page.
public function addCSS($styleSheet, $media = false, $position = false) 
{
...
// Star/unstar a conversation.
public function star($conversationId)
{
...
public function htmlStar($conversationId, $starred)
{
	global $language;
...
public function getAvatar($memberId, $avatarFormat, $type = false)
{
...
public function updateLastAction($action)
{
	if (!$this->user) return false;
...
public function updateUserRoleFlags($account = null)
{
	if (!$this->user) return;
...
public function changeMemberGroup($memberId, $newGroup, $currentGroup = false)
{
	global $config;
...
public function canChangeGroup($memberId, $group)
{
	global $config;
