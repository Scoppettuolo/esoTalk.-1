...
public $view = "conversation.view.php";
public $editingPost = false; // Are we editing a post (specified in $_GET["editPost"])?
public $showingDeletedPost = false; // Are we showing a delete post (specified in $_GET["showDeletedPost"])?
public $conversation = array(); // An array of conversation details.
public $startFrom = 0; // Which post to start viewing from.
...
public function getConversation($id = false)
{
	...
	if ($id !== false) {
		...
		$from = array(
			...
			"LEFT JOIN {$config["tablePrefix"]}status s ON (s.conversationId=c.conversationId AND s.memberId=" . ($this->eso->user ? (int)$this->eso->user["memberId"] : 0) . ")",
			"INNER JOIN {$config["tablePrefix"]}members sm ON (sm.memberId=c.startMember)"
		);

		$id = (int)$id;
		$where = array("c.conversationId=$id");
		$where[] = conversationAccessPredicate($this->eso, $config, "c", "s");
...
public function getPosts($criteria = array(), $display = false)
{
	global $language, $config;
	...
	$where = array("p.conversationId=" . (int)$this->conversation["id"]);
	if (isset($criteria["lastActionTime"])) $where[] = "time>" . (int)$criteria["lastActionTime"] . " OR editTime>" . (int)$criteria["lastActionTime"];
	if (isset($criteria["postIds"])) {
		$ids = sanitizeIdList($criteria["postIds"]);
		if ($ids) $where[] = "p.postId IN ($ids)";
		else $where[] = "1=0";
	}
	
	// Compile the query components.
	$components = array("select" => $select, "from" => $from, "where" => $where, "orderBy" => array("p.time ASC"), "limit" => $limit ? ((int)$startFrom . "," . (int)$limit) : "");
...
public function updateLastRead($lastRead)
{
...
public function addReply($content, $newConversation = false)
{
	global $config;
	...
	if (!$newConversation) {
		...
		if ($result) {
			while ($row = $this->eso->db->fetchRow($result)) {
				list($name, $email, $recipientLanguage) = $row;
			$languageFile = sanitizeFileName(file_exists(PATH_LANGUAGES . "/$recipientLanguage.php") ? $recipientLanguage : $config["language"]);
				include PATH_LANGUAGES . "/$languageFile.php";
...
// Save a draft.
public function saveDraft($content)
{
	global $language, $config;
...
public function discardDraft()
{
	...
	$this->eso->db->queryPrepared(
		"UPDATE {$config["tablePrefix"]}status SET draft=NULL WHERE conversationId=? AND memberId=?",
		"ii",
		(int)$this->conversation["id"],
		(int)$this->eso->user["memberId"]
	);
...
public function editPost($postId, $content)
{
	...
	$time = time();
	$memberId = (int)$this->eso->user["memberId"];
	if ($this->eso->db->isSQLite()) {
		$this->eso->db->queryPrepared("UPDATE {$config["tablePrefix"]}posts SET content=?, editMember=?, editTime=? WHERE postId=?", "siii", $content, $memberId, $time, $postId);
		$this->eso->db->queryPrepared("UPDATE {$config["tablePrefix"]}conversations SET lastActionTime=? WHERE conversationId=(SELECT conversationId FROM {$config["tablePrefix"]}posts WHERE postId=?)", "ii", $time, $postId);
	} else {
...
public function getEditControls($id)
{
	global $language, $config;
...
public function getEditArea($postId, $content)
{
	global $language;
	
	$html = "<form action='" . curLink() . "' method='post' enctype='multipart/form-data'><div class='widthFixer'>
<input type='hidden' name='token' value='{$_SESSION["token"]}'/>
<textarea cols='100' rows='4' id='p$postId-textarea' name='content' class='auto-expand'>$content</textarea>
...
public function formatForEditing($content)
{
	$formatters = array();
...
public function formatForDisplay($content)
{
	$formatters = array();
...
public function displayPost($content)
{
...
public function deletePost($postId)
{
	...
	if ($this->eso->db->isSQLite()) {
		if ($this->eso->user["moderator"]) {
			$this->eso->db->queryPrepared("UPDATE {$config["tablePrefix"]}posts SET deleteMember=?, editTime=? WHERE postId=?", "iii", $memberId, $time, $postId);
		} else {
			$this->eso->db->queryPrepared("UPDATE {$config["tablePrefix"]}posts SET deleteMember=?, editTime=? WHERE postId=? AND memberId=?", "iiii", $memberId, $time, $postId, $memberId);
		}
		$changed = $this->eso->db->affectedRows();
		$this->eso->db->queryPrepared("UPDATE {$config["tablePrefix"]}conversations SET lastActionTime=? WHERE conversationId=(SELECT conversationId FROM {$config["tablePrefix"]}posts WHERE postId=?)", "ii", $time, $postId);
...
public function restorePost($postId)
{
	...
	if ($this->eso->db->isSQLite()) {
		if ($this->eso->user["moderator"]) {
			$this->eso->db->queryPrepared("UPDATE {$config["tablePrefix"]}posts SET deleteMember=NULL, editMember=?, editTime=? WHERE postId=?", "iii", $memberId, $time, $postId);
		} else {
			$this->eso->db->queryPrepared("UPDATE {$config["tablePrefix"]}posts SET deleteMember=NULL, editMember=?, editTime=? WHERE postId=? AND memberId=?", "iiii", $memberId, $time, $postId, $memberId);
		}
		$changed = $this->eso->db->affectedRows();
		$this->eso->db->queryPrepared("UPDATE {$config["tablePrefix"]}conversations SET lastActionTime=? WHERE conversationId=(SELECT conversationId FROM {$config["tablePrefix"]}posts WHERE postId=?)", "ii", $time, $postId);
	} elseif ($this->eso->user["moderator"]) {
...
// Permanently delete a post.
public function deletePostForever($postId)
{
	global $config;
	$postId = (int)$postId;
	...
	if ($this->eso->db->isSQLite()) {
		$this->eso->db->queryPrepared("DELETE FROM {$config["tablePrefix"]}posts WHERE postId=?", "i", $postId);
	} else {
...
public function deleteConversation()
{
	...
	if ($this->eso->db->isSQLite()) {
		$this->eso->db->queryPrepared("DELETE FROM {$config["tablePrefix"]}tags WHERE conversationId=?", "i", $conversationId);
		$this->eso->db->queryPrepared("DELETE FROM {$config["tablePrefix"]}status WHERE conversationId=?", "i", $conversationId);
		$this->eso->db->queryPrepared("DELETE FROM {$config["tablePrefix"]}posts WHERE conversationId=?", "i", $conversationId);
		$this->eso->db->queryPrepared("DELETE FROM {$config["tablePrefix"]}conversations WHERE conversationId=?", "i", $conversationId);
	} else {
...
public function emailPrivateAdd($memberIds, $emailAll = false)
{
...
public function htmlMembersAllowedList($membersAllowed)
{
	global $language;
...
public function saveTitle($title)
{
	...
	$lastActionTime = time();
	$conversationId = (int)$this->conversation["id"];
	$query = $this->eso->db->isSQLite()
		? "UPDATE {$config["tablePrefix"]}conversations SET title=?, slug=?, lastActionTime=? WHERE conversationId=?"
		: "UPDATE {$config["tablePrefix"]}conversations c SET title=?, slug=?, lastActionTime=? WHERE c.conversationId=?";
	...
	$postTitleQuery = $this->eso->db->isSQLite()
		? "UPDATE {$config["tablePrefix"]}posts SET title=? WHERE conversationId=?"
		: "UPDATE {$config["tablePrefix"]}posts p SET title=? WHERE conversationId=?";
	$this->eso->db->queryPrepared($postTitleQuery, "si", $title, $conversationId);
}
...
public function removeQuotes($post)
{
...
public function canEditMembersAllowed()
{
	if ($this->eso->isSuspended()) return false;
...
public function canEditPost($postId, $memberId = false, $account = false, $deleteMember = -1)
{
	global $config;
	$postId = (int)$postId;
...
// Provide the post id or the post member, account, and deleteMember.
public function canDeletePost($postId, $memberId = false, $account = false, $deleteMember = -1)
{
	global $config;
	$postId = (int)$postId;
...
public function canDeleteConversation()
{
	global $config;
	$canDelete = false;
	if ($this->eso->user) {
