...
public function sendVerificationEmail($email, $name, $verifyHash)
{
	global $language, $config;
...
public function validateMember($hash)
{
	global $config;
	...
	if ($row) {
		...
		if (empty($config["requireManualApproval"])) {
			...
			if ($account == "Member") $this->eso->message("accountValidated", true, makeLink("new"));
		}
	}
}

// Add an element to the page's form.
public function addToForm($fieldset, $field, $position = false)
{
	return addToArray($this->form[$fieldset], $field, $position);
}

// Add a fieldset to the form.
public function addFieldset($fieldset, $legend, $position = false)
{
