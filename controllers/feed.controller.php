	...
	switch (@$_GET["q2"]) {
		...
		case "conversation":
			...
			if ($conversation["private"] or $conversation["posts"] == 0) {
				...
				if (!$this->eso->user) {
					header('WWW-Authenticate: Basic realm="esoTalk Atom feed"');
			...
			$result = $this->eso->db->fetchPrepared("SELECT postId, name, content, time FROM {$config["tablePrefix"]}posts INNER JOIN {$config["tablePrefix"]}members USING (memberId) WHERE conversationId=? AND deleteMember IS NULL ORDER BY time DESC LIMIT 20", "i", (int)$conversation["id"]);
