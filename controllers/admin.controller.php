...
public $view = "admin/admin.view.php";
public $subView = "";
...
public function dashboardInit(&$adminController)
{
...
public function dashboardAjax(&$adminController)
{
	global $config;
 		...
 		case "checkForUpdates":
 			if ($latestVersion = $this->eso->checkForUpdates()
				and ($this->user["memberId"] == $config["rootAdmin"]))
 					return $this->eso->htmlMessage("updatesAvailable", array($latestVersion, ESO_VERSION));
 	}
}

public function settingsInit(&$adminController)
{
...
public function changeLogo()
{
	...
	switch ($_POST["logo"]["type"]) {
		...
		case "none":
			...
			if (!empty($config["forumLogo"])) {
				$oldLogo = $config["forumLogo"];
				if (!preg_match('/^(?:[A-Za-z]:[\\\\\/]|[\\\\\/]{1,2})/', $oldLogo)) $oldLogo = PATH_ROOT . "/" . ltrim($oldLogo, "/\\\\");
	...
	$config["forumLogo"] = "config/" . basename($result["path"]);
...
public function changeIcon()
{
	...
	switch ($_POST["icon"]["type"]) {
		...
		case "none":
			...
			if (!empty($config["forumIcon"])) {
				$oldIcon = $config["forumIcon"];
				if (!preg_match('/^(?:[A-Za-z]:[\\\\\/]|[\\\\\/]{1,2})/', $oldIcon)) $oldIcon = PATH_ROOT . "/" . ltrim($oldIcon, "/\\\\");
	...
	$config["forumIcon"] = "config/" . basename($result["path"]);
	$this->writeSettingsConfig(array("forumIcon" => $config["forumIcon"]));
	
	return true;
}

public function writeSettingsConfig($newConfigElements)
{
	include PATH_CONFIG . "/config.php";

	// Recover a config rewritten by older versions whose Windows backslashes
	// were not escaped. Keep the database in the forum's config directory.
	if (isset($config["databaseDriver"]) && strtolower((string)$config["databaseDriver"]) === "sqlite" && isset($config["sqlitePath"]) && preg_match('/[\\x00-\\x1F\\x7F]/', (string)$config["sqlitePath"])) {
		$storedPath = (string)$config["sqlitePath"];
			$compactPath = preg_replace('/[^A-Za-z0-9._-]/', '', $storedPath);
			if (preg_match('/(?:esotalk|configsotalk)\\.sqlite$/i', $compactPath)) {
				// The old double-quoted serializer can turn "\\e" into ESC,
				// leaving the filename as configsotalk.sqlite.
		...
		$config["sqlitePath"] = "config/" . ($filename ?: "esotalk.sqlite");
	}

	$persistedConfig = array_merge($config, $newConfigElements);
	writeConfigFile(PATH_CONFIG . "/config.php", '$config', $persistedConfig);
	global $config;
	$config = $persistedConfig;
}

public function addSection($id, $title, $initFunction, $ajaxFunction = false, $position = false)
{
	addToArrayString($this->sections, $id, array("title" => $title, "initFunction" => $initFunction, "ajaxFunction" => $ajaxFunction), $position);
}

public function languagesInit()
{	
	global $language, $config;
...
public function installLanguage()
{
	...
	if (!move_uploaded_file($_FILES["installLanguage"]["tmp_name"], PATH_LANGUAGES . "/" . sanitizeFileName($_FILES["installLanguage"]["name"]))) {
...
public function membersInit(&$adminController)
{
	global $language, $config;
	...
	$this->unvalidated = $this->eso->db->fetchPrepared("SELECT memberId, name, avatarFormat, IF(color>?, ?, color), account, lastSeen, lastAction FROM {$config["tablePrefix"]}members WHERE account='Unvalidated' ORDER BY memberId DESC", "ii", $colors, $colors);
	$this->numberUnvalidated = $this->eso->db->numRows($this->unvalidated);
}

public function saveMembersSettings()
{
	$newConfig = array();
...
public function pluginsInit(&$adminController)
{
	...
	if ($handle = opendir(PATH_PLUGINS)) {
	    while (false !== ($file = readdir($handle))) {
	        ...
	        if ($file[0] != "." and is_dir(PATH_PLUGINS . "/$file") and file_exists(PATH_PLUGINS . "/$file/plugin.php") and (include_once PATH_PLUGINS . "/$file/plugin.php") and class_exists($file)) {
				$plugin = new $file;
				$plugin->eso =& $this->eso;
...
// Toggle a plugin.
public function togglePlugin($plugin)
{
	...
	if (!writeConfigFile(PATH_CONFIG . "/plugins.php", '$config["loadedPlugins"]', array_values((array)$config["loadedPlugins"]))) {
...
public function installPlugin()
{
	...
	if (!move_uploaded_file($_FILES["installPlugin"]["tmp_name"], PATH_PLUGINS . "/" . sanitizeFileName($_FILES["installPlugin"]["name"]))) {
		$this->eso->message("notWritable", false, "plugins/");
		return false;
	}
	
	// Unzip the plugin. If we can't, show an error.
	if (!($files = unzip(PATH_PLUGINS . "/" . sanitizeFileName($_FILES["installPlugin"]["name"]), PATH_PLUGINS . "/"))) $this->eso->message("invalidPlugin");
	else {
		...
		if ($pluginFound and $directories == 1) {
			...
			foreach ($files as $k => $file) {
				...
				if ($file["directory"] and !is_dir(PATH_PLUGINS . "/{$file["name"]}")) mkdir(PATH_PLUGINS . "/{$file["name"]}");
				
				// Write a file.
				elseif (!$file["directory"]) {
					if (!writeFile(PATH_PLUGINS . "/{$file["name"]}", $file["content"])) {
	...
	unlink(PATH_PLUGINS . "/" . sanitizeFileName($_FILES["installPlugin"]["name"]));
}

public $skins = array();

// Get all the skins into an array.
public function skinsInit()
{	
	global $language, $config;
	...
	if ($handle = opendir(PATH_SKINS)) {
	    while (false !== ($file = readdir($handle))) {
	        ...
	        if ($file[0] != "." and is_dir(PATH_SKINS . "/$file") and file_exists(PATH_SKINS . "/$file/skin.php") and (include_once PATH_SKINS . "/$file/skin.php") and class_exists($file)) {
	        	$skin = new $file;
				if (file_exists(PATH_SKINS . "/$file/preview.jpg")) $preview = "preview.jpg";
				elseif (file_exists(PATH_SKINS . "/$file/preview.png")) $preview = "preview.png";
				elseif (file_exists(PATH_SKINS . "/$file/preview.gif")) $preview = "preview.gif";
				else $preview = "";
...
public function changeSkin($skin)
{
	...
	writeConfigFile(PATH_CONFIG . "/skin.php", '$config["skin"]', $skin);
...
public function installSkin()
{
	...
	if (!move_uploaded_file($_FILES["installSkin"]["tmp_name"], PATH_SKINS . "/" . sanitizeFileName($_FILES["installSkin"]["name"]))) {
		$this->eso->message("notWritable", false, "skins/");
		return false;
	}

	// Unzip the skin. If we can't, show an error.
	if (!($files = unzip(PATH_SKINS . "/" . sanitizeFileName($_FILES["installSkin"]["name"]), PATH_SKINS . "/"))) $this->eso->message("invalidSkin");
	else {
		...
		if ($skinFound and $directories == 1) {
			...
			foreach ($files as $k => $file) {
				...
				// Make a directory if it doesn't exist!
				if ($file["directory"] and !is_dir(PATH_SKINS . "/{$file["name"]}")) mkdir(PATH_SKINS . "/{$file["name"]}");

				// Write a file.
				elseif (!$file["directory"]) {
					if (!writeFile(PATH_SKINS . "/{$file["name"]}", $file["content"])) {
	...
	unlink(PATH_SKINS . "/" . sanitizeFileName($_FILES["installSkin"]["name"]));
}
